...
export async function generatePhase2Content(options: GenerateOptions): Promise<Phase2ContentPack> {
  ...
  try {
    ...
    return {
      ...
      videoPrompts: Array.isArray(parsed.videoPrompts) ? parsed.videoPrompts : (parsed.videoPrompts ? [parsed.videoPrompts] : generateDefaultVideoPrompts(options)),
      imagePrompts: Array.isArray(parsed.imagePrompts) ? parsed.imagePrompts : (parsed.imagePrompts ? [parsed.imagePrompts] : generateDefaultImagePrompts(options)),
